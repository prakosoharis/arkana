...
**Updated:** 2026-08-26 — ARK-S21-01 complete; Owner acceptance pending
**Active milestone:** Sprint 21 — Journal, Controlled Learning, and LIVE-Readiness Governance
**Active card:** ARK-S21-01 — Immutable unified journal index and lineage
...
| Journal / controlled learning / LIVE-readiness | ARK-S21-01 COMPLETE; OWNER ACCEPTANCE PENDING | Migration 047 and `GOVERNANCE_JOURNAL_INDEX_V1` implement a 23-type append-only exact-reference index, deterministic origin/scope/time semantics, source snapshot and lineage tamper detection, privacy-safe account references, cursor pagination, read-only verifier, FastAPI, and BFF. Runtime journal remains honestly zero because there is no implicit backfill. Incident/recovery, learning, readiness, UI, and LIVE authority do not yet exist. |
