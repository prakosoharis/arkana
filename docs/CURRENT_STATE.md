...
**Active card:** ARK-S15-03 — complete, awaiting Owner acceptance
...
| Variant Explorer | HOLDOUT EVIDENCE + LOCK — ARK-S15-03 | The immutable S15-01 contract and accepted train matrix now feed holdout-only nominal/adverse execution through the canonical evaluator. Exact baseline parity, deltas, comparison classes, absolute eligibility, deterministic ranking, and an immutable selection lock exist. Runtime truth is `NO_ELIGIBLE_VARIANT`; final-OOS and revision/lifecycle integration remain unimplemented. |
...
pushed at `1fdc28c`; only ARK-S15-03 is currently authorized and is complete
awaiting Owner acceptance. Its runtime lock truthfully records
`NO_ELIGIBLE_VARIANT`: all challengers remained negative under nominal and
adverse holdout costs. Final-OOS remains untouched, and exploration cannot
create `VALIDATED`, DEMO, LIVE, Router, or trading-decision claims.
