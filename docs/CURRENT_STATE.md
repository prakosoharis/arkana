...
**Updated:** 2026-08-24 — ARK-S14-02 deterministic fixed-lot equity engine
**Active milestone:** Sprint 14 — Broker-Realistic Historical Capital Simulation
**Active card:** ARK-S14-02 — complete, awaiting Owner acceptance
...
| Capital Simulation | FIXED-LOT ENGINE IMPLEMENTED; constraints incomplete | Immutable `CAPITAL_BROKER_CONTRACT_V1` records exact StrategyVersion, MT5 broker snapshot/parity, capital, sizing, leverage, margin, and failure assumptions. `FIXED_LOT_REALIZED_EQUITY_V1` reuses the sole canonical kernel, verifies exact full-history traversal invariants, converts after-cost price PnL through frozen MT5 tick values, and persists a paginable realized-equity path. Fractional sizing, compounding, margin, liquidation, and intratrade mark-to-market remain unimplemented. |
...
Simulation**. ARK-S14-01 is accepted and was pushed at commit `52fd5e9`.
The current card is **ARK-S14-02**: deterministic fixed-lot realized-equity
simulation, implemented and awaiting Owner acceptance. Its source contract,
runtime evidence, limits, and Owner OAT are recorded in
`docs/SPRINT_14_CAPITAL_SIMULATION.md`. ARK-S14-03 has not started.
