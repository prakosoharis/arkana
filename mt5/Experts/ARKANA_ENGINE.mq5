...
struct GenericConfig {
  ...
  string target_rule; string target_distance; string atr_period;
  string stop_atr_multiplier; string target_atr_multiplier;
  string spread_guard; string max_spread_price;
  string max_open_positions; string session_clock; string session_windows;
  string ambiguity_policy; string emergency_stop_source;
...
bool AssignGenericField(GenericConfig &cfg,const string key,const string value) {
  ...
  else if(key=="target_rule") cfg.target_rule=value; else if(key=="target_distance") cfg.target_distance=value; else if(key=="atr_period") cfg.atr_period=value;
  else if(key=="stop_atr_multiplier") cfg.stop_atr_multiplier=value; else if(key=="target_atr_multiplier") cfg.target_atr_multiplier=value; else if(key=="spread_guard") cfg.spread_guard=value;
...
// ARK-S24-03. Exactly one distance model is in force; the other is NONE.  The
// terminal never falls back to a default distance: a scaled config with too
// little completed history refuses the entry instead.
bool g_atr_scaled=false; int g_atr_period=0;
double g_stop_multiplier=0.0; double g_target_multiplier=0.0;
bool ParseDistanceModel(const GenericConfig &cfg) {
  g_atr_scaled=false; g_atr_period=0; g_stop_multiplier=0.0; g_target_multiplier=0.0;
  if(cfg.stop_rule=="FIXED_PRICE_DISTANCE_SL") {
    if(cfg.target_rule!="FIXED_PRICE_DISTANCE_TP") return false;
    if(cfg.atr_period!="NONE" || cfg.stop_atr_multiplier!="NONE" || cfg.target_atr_multiplier!="NONE") return false;
    if(DoubleToString(StringToDouble(cfg.stop_distance),8)!=cfg.stop_distance) return false;
    if(DoubleToString(StringToDouble(cfg.target_distance),8)!=cfg.target_distance) return false;
    return StringToDouble(cfg.stop_distance)>0 && StringToDouble(cfg.target_distance)>0;
  }
  if(cfg.stop_rule!="ATR_SCALED_SL" || cfg.target_rule!="ATR_SCALED_TP") return false;
  if(cfg.stop_distance!="NONE" || cfg.target_distance!="NONE") return false;
  if(!IsDecimalDigits(cfg.atr_period)) return false;
  if(DoubleToString(StringToDouble(cfg.stop_atr_multiplier),8)!=cfg.stop_atr_multiplier) return false;
  if(DoubleToString(StringToDouble(cfg.target_atr_multiplier),8)!=cfg.target_atr_multiplier) return false;
  int period=(int)StringToInteger(cfg.atr_period);
  if(period<=0 || period>1000 || IntegerToString(period)!=cfg.atr_period) return false;
  if(StringToDouble(cfg.stop_atr_multiplier)<=0 || StringToDouble(cfg.target_atr_multiplier)<=0) return false;
  g_atr_scaled=true; g_atr_period=period;
  g_stop_multiplier=StringToDouble(cfg.stop_atr_multiplier);
  g_target_multiplier=StringToDouble(cfg.target_atr_multiplier);
  return true;
}
// Mean true range over completed candles only.  `rates` is series-ordered, so
// index 1 is the completed signal bar and index period+1 supplies the previous
// close the oldest true range needs.  Index 0 is the forming bar and is never
// read, which is what keeps the terminal free of look-ahead.
bool CompletedAtr(const MqlRates &rates[],const int period,double &value) {
  value=0.0;
  if(period<=0 || ArraySize(rates)<period+2) return false;
  double total=0.0;
  for(int index=1;index<=period;index++) {
    double high=rates[index].high,low=rates[index].low,previous=rates[index+1].close;
    double range=high-low;
    double up=MathAbs(high-previous),down=MathAbs(low-previous);
    if(up>range) range=up;
    if(down>range) range=down;
    total+=range;
  }
...
bool ReadGenericConfig(const string file,GenericConfig &cfg) {
  ...
  FileClose(handle);
  string required[]={"schema_version","compiler_protocol_version","adapter_capability_id","generic_demo_contract_id","generic_demo_contract_fingerprint","strategy_version_id","strategy_checksum","canonical_instrument","broker_symbol","enabled","allowed_environment","direction","execution_timeframe","context_rule","context_timeframe","sma_fast_period","sma_slow_period","sma_relation","setup_rule","setup_timeframe","setup_direction","trigger_rule","trigger_timeframe","trigger_direction","entry_rule","entry_price_source","uses_completed_candles","uses_future_ohlc","invalidation_rule","volume","stop_rule","stop_distance","target_rule","target_distance","atr_period","stop_atr_multiplier","target_atr_multiplier","spread_guard","max_spread_price","max_open_positions","session_clock","session_windows","ambiguity_policy","emergency_stop_source","emergency_stop_variable","emergency_stop_condition","emergency_stop_action","force_close_positions","checksum"};
  ...
  if(cfg.uses_completed_candles!="true" || cfg.uses_future_ohlc!="false" || cfg.invalidation_rule!="ALWAYS") return false;
  if(!ParseDistanceModel(cfg)) return false;
  ...
  if(fast<=0 || slow<=fast || slow>1000 || DoubleToString(StringToDouble(cfg.volume),8)!=cfg.volume || DoubleToString(StringToDouble(cfg.max_spread_price),8)!=cfg.max_spread_price) return false;
  return StringToDouble(cfg.volume)>0 && StringToDouble(cfg.max_spread_price)>0;
}
...
void GenericOnNewBar() {
  ...
  // ARK-S24-03: a scaled distance needs its own completed lookback, so the
  // copy covers whichever of the two rules reaches further back.
  int needed=slow+2; if(g_atr_scaled && g_atr_period+2>needed) needed=g_atr_period+2;
  MqlRates rates[]; ArraySetAsSeries(rates,true); if(CopyRates(_Symbol,PERIOD_M1,0,needed,rates)<needed) { EmitGenericSimple("BLOCKER","COMPLETED_BARS_UNAVAILABLE"); return; }
  ...
  // ARK-S24-03: a scaled contract with too little completed history refuses the
  // signal, exactly as the evaluator does.  It never falls back to a distance.
  double signal_atr=0.0;
  if(g_atr_scaled && !CompletedAtr(rates,g_atr_period,signal_atr)) { EmitGenericSimple("BLOCKER","ATR_UNAVAILABLE"); return; }
  ...
  double stop_distance=g_atr_scaled?g_stop_multiplier*signal_atr:StringToDouble(active_generic.stop_distance);
  double target_distance=g_atr_scaled?g_target_multiplier*signal_atr:StringToDouble(active_generic.target_distance);
