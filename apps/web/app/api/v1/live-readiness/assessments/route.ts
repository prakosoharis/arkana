...
export async function GET(request: NextRequest) {
  try {
    const response = await fetch(`${base()}/api/v1/live-readiness/assessments?${request.nextUrl.searchParams.toString()}`, { cache: "no-store" });
...
export async function POST(request: NextRequest) {
  try {
    const response = await fetch(`${base()}/api/v1/live-readiness/assessments`, {
