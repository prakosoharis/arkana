...
def test_strategy_factory_migration_preserves_legacy_and_supports_pre_backtest_lineage(tmp_path):
    ...
    assert "live_readiness_assessments" in metadata.get_table_names()
    assert "sprint21_acceptance_verifications" in metadata.get_table_names()
    assert {"edge_search_campaigns", "edge_search_trials", "edge_search_final_oos_openings"}.issubset(metadata.get_table_names())
    ...
    with engine.connect() as connection:
        ...
        assert connection.execute(text("SELECT COUNT(*) FROM schema_migrations WHERE version = :version"), {"version": MIGRATION_050}).scalar_one() == 1
        assert connection.execute(text("SELECT COUNT(*) FROM schema_migrations WHERE version = :version"), {"version": MIGRATION_051}).scalar_one() == 1
        assert connection.execute(text("SELECT COUNT(*) FROM schema_migrations WHERE version = :version"), {"version": MIGRATION_052}).scalar_one() == 1
