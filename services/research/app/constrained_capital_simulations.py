...
def verify_full_history(session: Session, item: ConstrainedCapitalSimulation) -> dict:
    ...
    try:
        ...
        if source_trades:
            ...
            # ARK-S25-04: walk only as far as the source validation itself
            # recorded.  A registered dataset grows -- an MT5 sync appended
            # 11,281 bars -- and replaying the whole live asset produced more
            # trades than the stored capital path has points, reporting an
            # untouched record as "capital path truncated".
            recorded_bars = int((full.result or {}).get("bars_evaluated") or 0)
            def _bounded(source, limit):
                seen = 0
                for chunk in source:
                    if seen + len(chunk) <= limit:
                        seen += len(chunk); yield chunk
                    else:
                        yield chunk[:limit - seen]; return
            stream = _bounded(iter_bars(asset), recorded_bars) if recorded_bars else iter_bars(asset)
            simulate_kernel(stream, full.configuration, on_trade=lambda trade: compare_point(accumulator.observe(trade)))
