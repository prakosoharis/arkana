...
@app.post("/api/v1/governance/sprint23-acceptance-verifications")
def post_sprint23_acceptance_verification(session: Session = Depends(get_session)) -> dict:
    try:
        item, reused = materialize_sprint23_acceptance(session)
        return serialize_sprint23_acceptance(item, reused=reused)
    except ValueError as error:
        raise HTTPException(422, str(error)) from error


@app.get("/api/v1/governance/sprint23-acceptance-verifications/latest")
def get_latest_sprint23_acceptance_verification(session: Session = Depends(get_session)) -> dict:
    item = latest_sprint23_acceptance(session)
    if not item:
        raise HTTPException(404, "Sprint 23 acceptance verification has not been materialized")
    return serialize_sprint23_acceptance(item)


@app.get("/api/v1/governance/sprint23-acceptance-verifications/{verification_id}/verification")
def get_sprint23_acceptance_verification(verification_id: str, session: Session = Depends(get_session)) -> dict:
    item = session.get(Sprint23AcceptanceVerification, verification_id)
    if not item:
        raise HTTPException(404, "Sprint 23 acceptance verification not found")
    return verify_sprint23_acceptance(session, item)
...
def get_market_explorer_timeframes(session: Session = Depends(get_session)) -> dict:
    ...
    return {"symbol": dataset.symbol, "dataset": {"id": dataset.id, "fingerprint": dataset.fingerprint,
            ...
            "timezones": list(MARKET_EXPLORER_TIMEZONES),
            "clock": market_exploration_clock("WIB", dataset),
            "timeframes": [{"timeframe": timeframe, "rows": registered[timeframe].row_count,
                            "measured": {zone: market_exploration_cached(session, dataset, timeframe, zone) is not None
                                         for zone in MARKET_EXPLORER_TIMEZONES}}
                           for timeframe in MARKET_EXPLORER_TIMEFRAMES if timeframe in registered]}


@app.get("/api/v1/market-explorer/{timeframe}")
def get_market_exploration(timeframe: str, timezone: str = "WIB", refresh: bool = False,
                           session: Session = Depends(get_session)) -> dict:
    try:
        record, reused = measure_market(session, timeframe=timeframe.upper(),
                                        display_timezone=timezone.upper(), refresh=refresh)
    except ValueError as error:
        raise HTTPException(422, str(error)) from error
